...
async function getvendors(page = 1) {
    try{
        ...
        paginate.value = response.data.vendors;
    }catch(error){
        Swal.fire({
            title: error.response.status,
            text: 'Terjadi kesalahan saat mengambil data vendors 🚫',
            icon: 'error',
...
async function edit() {
    try{
        ...
        const response = await axios.patch(`/api/master/vendor/${form.value.id}`, form.value);
        openModalUpdate.value = false;
        reset();
        getvendors();
        Swal.fire({
            title: 'Berhasil!',
            text: 'Data vendor berhasil dirubah',
            icon: 'success',
...
async function hapus() {
    try{
        ...
        const response = await axios.delete(`/api/master/vendors/${form.value.id}`);
        Swal.fire({
            title: 'Berhasil!',
...
function detail(vendor){
    form.value.id = vendor.id;
    form.value.nama = vendor.nama;
    form.value.alamat = vendor.alamat;
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                <template v-if="vendors.length > 0">
                                    <tr v-for="(vendor, index) in vendors" :key="index" class="border-b dark:border-gray-700">
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ paginate.from + index }}</th>
                                        <td class="px-4 py-3">{{ vendor.nama }}</td>
                                        <td class="px-4 py-3">{{ vendor.alamat }}</td>
                                        <td class="px-4 py-3 flex items-center justify-end">
                                            ...
                                            <div v-if="index === menu" class="z-10 w-44 absolute bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                                <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                                    <li>
                                                        <button type="button" @click.stop="detail(vendor)" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                    ...
                                                    <li>
                                                        <button type="button" @click="del(vendor)" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
                                ...
                                <template v-else>
                                    <tr>
                                        <td colspan="4">Data vendors tidak ada</td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
                ...
                <div class="relative p-4 w-full max-w-2xl max-h-full">
                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                            ...
                            <button type="button" @click="openModalCreate = false; reset; getvendors()" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-target="createProductModal" data-modal-toggle="createProductModal">
                        ...
                        <form @submit.prevent="create">
                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <input ref="createNameInput" @keydown.enter.prevent="createAlamatInput.focus()" v-model="form.nama" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Nama">
                                ...
                                <div>
                                    ...
                                    <input ref="createAlamatInput" @keydown.enter.prevent="create" v-model="form.alamat" type="text" name="brand" id="brand" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Alamat">
            ...
            <div v-if="openModalUpdate" tabindex="-1" aria-hidden="true" class="overflow-y-auto bg-slate-900/50 h-screen overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 max-h-full flex inset-0">
                <div class="relative p-4 w-full max-w-2xl max-h-full">
                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        <!-- Modal header -->
                        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Update Vendor</h3>
                        ...
                        <form @submit.prevent="edit">
                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                <div>
                                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama</label>
                                    <input v-model="form.nama" @keydown.enter.prevent="editAlamatInput.focus()" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Nama">
                                    <p v-if="err.nama" class="text-xs text-red-400">{{ err.nama[0] }}</p>
                                </div>
                                <div>
                                    <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Alamat</label>
                                    <input v-model="form.alamat" @keydown.enter.prevent="edit" type="text" name="brand" id="brand" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Alamat">
                                    <p v-if="err.alamat" class="text-xs text-red-400">{{ err.alamat[0] }}</p>
                                </div>
                            </div>
                            <button type="submit" :disabled="isloadedit" :class="isloadedit ? 'opacity-50 cursor-not-allowed' : 'opacity-100 cursor-pointer'" class="text-white inline-flex items-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                ...
                                {{ isloadedit ? 'Mengedit vendor' : 'Edit vendor' }}
                            </button>
                        </form>
                    </div>
                </div>
