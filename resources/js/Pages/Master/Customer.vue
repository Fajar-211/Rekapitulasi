...
const createfirstNameInput = ref(null)
const createlastNameInput = ref(null)
...
async function getcustomers(page = 1) {
    try{
        ...
        paginate.value = response.data.customers;
    }catch(error){
        Swal.fire({
...
async function create() {
    try{
        ...
        const response = await axios.post('/api/master/customer', form.value);
        reset();
        await nextTick()
        createfirstNameInput.value?.focus()
        //getcustomers();
    }catch(error){
...
async function edit() {
    try{
        ...
        const response = await axios.patch(`/api/master/customer/${form.value.id}`, form.value);
...
async function hapus() {
    try{
        ...
        const response = await axios.delete(`/api/master/customers/${form.value.id}`);
        Swal.fire({
            title: 'Berhasil!',
            text: 'Berhasil menghapus customer',
            icon: 'success',
            confirmButtonText: 'OK'
        })
...
function detail(customer){
    ...
    form.value.nama = customer.nama_depan;
    form.value.last = customer.nama_belakang;
...
watch(openModalUpdate, async (val) => {
    if (val) {
        await nextTick()
        editfirstNameInput.value?.focus()
    }
})
</script>
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-4 py-3">Nama Depan</th>
                                    <th scope="col" class="px-4 py-3">Nama Belakang</th>
                            ...
                            <tbody>
                                <template v-if="customers.length > 0">
                                    <tr v-for="(customer, index) in customers" :key="index" class="border-b dark:border-gray-700">
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ paginate.from + index }}</th>
                                        <td class="px-4 py-3">{{ customer.nama_depan }}</td>
                                        <td class="px-4 py-3">{{ customer.nama_belakang }}</td>
                                        <td class="px-4 py-3 flex items-center justify-end">
                                            ...
                                            <div v-if="index === menu" class="z-10 w-44 absolute bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                                <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                                    <li>
                                                        <button type="button" @click.stop="detail(customer)" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                    ...
                                                    <li>
                                                        <button type="button" @click="del(customer)" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
                                ...
                                <template v-else>
                                    <tr>
                                        <td colspan="4">Data customers tidak ada</td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
                ...
                <div class="relative p-4 w-full max-w-2xl max-h-full">
                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5 dark:border-gray-600">
                            ...
                            <button type="button" @click="openModalCreate = false; reset; getcustomers()" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-target="createProductModal" data-modal-toggle="createProductModal">
                        ...
                        <form @submit.prevent="create">
                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                ...
                                <div>
                                    <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Belakang</label>
                                    <input ref="createlastNameInput" @keydown.enter.prevent="create" v-model="form.last" type="text" name="brand" id="brand" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Nama belakang">
                                    <p v-if="err.last" class="text-xs text-red-400">{{ err.last[0] }}</p>
                                </div>
                            </div>
                    ...
                    <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                        ...
                        <form @submit.prevent="edit">
                            <div class="grid gap-4 mb-4 sm:grid-cols-2">
                                <div>
                                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Depan</label>
                                    <input v-model="form.nama" @keydown.enter.prevent="editlastNameInput.focus()" type="text" name="name" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Nama depan">
                                    <p v-if="err.nama" class="text-xs text-red-400">{{ err.nama[0] }}</p>
                                </div>
                                <div>
                                    <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Belakang</label>
                                    <input v-model="form.last" @keydown.enter.prevent="edit" type="text" name="brand" id="brand" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Nama belakang">
                                    <p v-if="err.last" class="text-xs text-red-400">{{ err.last[0] }}</p>
                                </div>
                            </div>
                            <button type="submit" :disabled="isloadedit" :class="isloadedit ? 'opacity-50 cursor-not-allowed' : 'opacity-100 cursor-pointer'" class="text-white inline-flex items-center bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                                ...
                                {{ isloadedit ? 'Mengedit customer' : 'Edit customer' }}
                            </button>
                        </form>
                    </div>
                </div>
